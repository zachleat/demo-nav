const AVATAR = "https://v1.indieweb-avatar.11ty.dev/";
	...
	let allProjectsBox = document.getElementById("f-all-projects");
	...
	function renderProjectOptions() {
		...
		let previous = projectSelect.value;
		let groups = new Map();
		for(let [ index, project ] of projects.entries()) {
			// Archived and deprecated projects are always hidden; the rest need the opt-in unless they're on npm.
			if(project.isArchived || project.npmDeprecated || (project.npmStatus !== "published" && !allProjectsBox.checked)) {
				continue;
			}
			if(!groups.has(project.owner)) {
				groups.set(project.owner, []);
			}
			groups.get(project.owner).push(new Option(projectName(project), index));
		}
	...
	allProjectsBox.addEventListener("change", renderProjectOptions);
